...
if (loaderElImmediate) loaderElImmediate.style.display = 'none';
        ...
        const viewH = isMobileView ? window.innerHeight * 0.5 : window.innerHeight - 64;
        ...
        gltfLoader.load(
            ...
            (gltf) => {
                ...
                // Expose to window for mobile adjustment
                window.__sashaThreeScene = { camera, model };
                // Mobile scale adjustment
                if (window.innerWidth <= 768) {
                    const mobileScale = baseScale * 0.22;
                ...
                // Mobile scale adjustment
                if (window.innerWidth <= 768) {
                    model.scale.setScalar(baseScale * 0.5);
                    camera.position.set(0, 1.0, 10);
                    camera.fov = 30;
                    camera.updateProjectionMatrix();
                }
        ...
        function animate() {
            ...
            if (model) {
                ...
                if (window.innerWidth <= 768) {
                    model.scale.setScalar(model.userData.baseScale * pulse);
                } else {
...
// Mobile canvas fix - move canvas inside hero section
(function() {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', fixMobileCanvas);
    } else {
        fixMobileCanvas();
    }
    setTimeout(fixMobileCanvas, 1000);
})();

// Mobile model enhancement
(function() {
    function enhanceMobileModel() {
        if (window.innerWidth > 768) return;
        ...
        // Fix hero section to contain canvas
        const hero = document.getElementById('home');
        if (hero) {
            hero.style.position = 'relative';
            hero.style.overflow = 'hidden';
        ...
        // Fix hero sticky to be above canvas
        const heroSticky = document.querySelector('.hero-sticky');
        if (heroSticky) {
            heroSticky.style.position = 'relative';
            heroSticky.style.zIndex = '2';
            heroSticky.style.paddingTop = '55vh';
            heroSticky.style.background = 'none';
        }
    }
    
    setTimeout(enhanceMobileModel, 500);
    setTimeout(enhanceMobileModel, 2000);
})();

// Mobile model scale and camera fix
(function() {
    function fixMobileModel() {
        if (window.innerWidth > 768) return;
        ...
        // Try to access Three.js camera and scale through window
        if (window.__sashaThreeScene) {
            const { camera, model } = window.__sashaThreeScene;
            if (camera) {
                camera.position.set(0, 0.5, 3.5); // closer on mobile
                camera.fov = 45;
                camera.updateProjectionMatrix();
            }
            if (model) {
                const currentScale = model.userData.baseScale || 1;
                model.scale.setScalar(currentScale * 1.5); // bigger on mobile
            }
        }
    }
    setTimeout(fixMobileModel, 2000);
    setTimeout(fixMobileModel, 4000);
})();
...
// Center model on mobile
(function() {
    function centerMobileModel() {
        if (window.innerWidth > 768) return;
        const canvas = document.getElementById('model-canvas');
        if (!canvas) return;
    ...
    setTimeout(centerMobileModel, 300);
    setTimeout(centerMobileModel, 1500);
    window.addEventListener('resize', centerMobileModel);
})();
